...
class TestMinTradesLogisticPenalty:
    ...
    def test_zero_trades_heavy_penalty(self):
        fitness = self.evaluator._apply_penalties(
            1.0, {"num_trades": 0, "max_drawdown": 0, "win_rate": 0.5}
        )
    ...
    def test_above_min_no_penalty(self):
        fitness = self.evaluator._apply_penalties(
            1.0, {"num_trades": 20, "max_drawdown": 0, "win_rate": 0.5}
        )
    ...
    def test_smooth_ramp_from_1_to_min(self):
        ...
        values = []
        for n in range(1, 11):
            f = self.evaluator._apply_penalties(
                1.0, {"num_trades": n, "max_drawdown": 0, "win_rate": 0.5}
            )
            values.append(f)
...
class TestDrawdownSigmoidPenalty:
    ...
    def test_smooth_around_threshold(self):
        ...
        for dd in [0.20, 0.25, 0.28, 0.30, 0.32, 0.35, 0.40]:
            f = self.evaluator._apply_penalties(
                1.0, {"num_trades": 50, "max_drawdown": dd, "win_rate": 0.5}
            )
            values.append(f)
        for i in range(1, len(values)):
...
class TestWinRateSigmoidPenalty:
    ...
    def test_low_trades_reduce_confidence(self):
        ...
        f_many = self.evaluator._apply_penalties(
            1.0, {"num_trades": 50, "max_drawdown": 0, "win_rate": 0.10}
        )
        f_few = self.evaluator._apply_penalties(
            1.0, {"num_trades": 2, "max_drawdown": 0, "win_rate": 0.10}
        )
    ...
    def test_one_trade_minimal_penalty(self):
        ...
        fitness = self.evaluator._apply_penalties(
            1.0, {"num_trades": 1, "max_drawdown": 0, "win_rate": 0.0}
        )
...
class TestDeadExitProportionalPenalty:
    ...
    def test_no_dead_exits_no_penalty(self):
        ...
        gene_alive = _make_gene(
            exit_conditions=[
                ConditionGene(indicator="RSI_0", operator=">", threshold=70, logic="AND"),
            ]
        )
        gene_dead = _make_gene(
            exit_conditions=[
    ...
    def test_all_dead_exits_max_penalty(self):
        ...
        gene = _make_gene(
            exit_conditions=[
                ConditionGene(
                    indicator="RSI_0", operator="<", threshold=0, logic="AND"
                ),  # dead: RSI < 0
            ]
        )
        fitness = self.evaluator._apply_penalties(
            1.0, {"num_trades": 50, "max_drawdown": 0, "win_rate": 0.5}, gene
        )
...
class TestCDLOperatorFallback:
    ...
    def test_cdl_operator_100_mutations(self):
        ...
        for _ in range(100):
            ...
            for cond in (
                mutated.strategy_gene.entry_conditions + mutated.strategy_gene.exit_conditions
            ):
...
class TestDiversityAwareElitism:
    ...
    def test_identical_individuals_only_one_survives(self):
        ...
        for candidate in ranked:
            if len(elites) >= 3:
                break
            too_close = any(calculate_strategy_distance(candidate, e) < threshold for e in elites)
            if not too_close:
...
class TestNSGA2EnvironmentalSelection:
    ...
    def test_crowding_preserves_extremes(self):
        ...
        assert max(distances) == float("inf"), (
...
class TestWorkerLogLevelConfigurable:
    ...
    def test_source_uses_config_worker_log_level(self):
        ...
        assert "worker_log_level" in source, "parallel.py should read worker_log_level from config"
...
class TestOutputDirFromConfig:
    """Verify run_ga.py derives output_dir from config, never ambient state."""

    def test_output_dir_from_config(self):
        """output.dir in config should be used when set."""
        from genetic_algorithm.run_ga import resolve_output_dir

        config = {"output": {"dir": "/tmp/test_ga_output_dir"}}
        output_dir = resolve_output_dir(config)
        assert str(output_dir) == "/tmp/test_ga_output_dir"

    def test_output_dir_env_cannot_override_hash_bound_config(self):
        """GA_OUTPUT_DIR is a removed alias and must not override config."""
        from genetic_algorithm.run_ga import resolve_output_dir

        config = {"output": {"dir": "/tmp/from_config"}}
        with patch.dict(os.environ, {"GA_OUTPUT_DIR": "/tmp/from_env"}):
            output_dir = resolve_output_dir(config)
        assert str(output_dir) == "/tmp/from_config"

    def test_output_dir_default_fallback(self):
        """Falls back to OUTPUT_DIR when config and env are absent."""
        from genetic_algorithm.run_ga import OUTPUT_DIR, resolve_output_dir

        assert resolve_output_dir({}) == OUTPUT_DIR
    ...
    def test_writes_valid_json(self, tmp_path):
        ...
        stats = [
            ...
            PopulationStats(
                ...
                genetic_diversity=0.58,
                best_raw_fitness=0.62,
...
class TestEvolutionDiagnosticsKeyFix:
    ...
    def test_reads_dir_key(self):
        ...
        assert (
            '"dir"' in source or "'dir'" in source
...
class TestC1OffspringLoopSafety:
    ...
    def test_evolution_source_has_max_attempts_guard(self):
        ...
        assert "max_attempts" in source, "_create_offspring() missing max_attempts guard"
        assert "attempts" in source, "_create_offspring() missing iteration counter"
...
class TestC3FeeNoiseDeterminism:
    ...
    def test_fee_noise_deterministic_same_inputs(self):
        ...
        assert results[0] == results[1] == results[2], f"Seeded RNG not deterministic: {results}"
    ...
    def test_fee_noise_no_name_error_at_runtime(self):
        ...
        assert any("strategy_name" in line and "fee_rng_seed" in line for line in lines), (
            "fee_rng_seed should use strategy_name for deterministic seeding"
        )


class TestC4TempFileCleanup:
    """C4: Compatibility runner must not create an untracked temp run."""

    def test_finally_block_in_main(self):
        """run_ga.main delegates before the disabled legacy temp-file path."""
        import inspect
        from genetic_algorithm.run_ga import main

        source = inspect.getsource(main)
        assert "cli_main" in source
        assert "_legacy_main_disabled" not in source
...
class TestH4DSRTrialsNonDuplicate:
    ...
    def test_fallback_only_when_no_hashes(self):
        ...
        tracker.register_evaluation(strategy_hash=None)
        tracker.register_evaluation(strategy_hash=None)
        assert tracker.n_trials == 3, f"Expected n_trials=3 (fallback only), got {tracker.n_trials}"
    ...
    def test_deduplication_on_same_hash(self):
        ...
        tracker.register_evaluation(strategy_hash="same")
        assert tracker.n_trials == 1, f"Expected n_trials=1 (dedup), got {tracker.n_trials}"
    ...
    def test_removal_blocked_when_only_exit_references_indicator(self):
        ...
        gene = StrategyGene(
            ...
            indicators=[
                IndicatorGene(type="RSI", parameters={"period": 14}, instance_id="RSI_0"),
                IndicatorGene(
                    type="MACD", parameters={"fast": 12, "slow": 26}, instance_id="MACD_0"
                ),
            ],
            entry_conditions=[
...
class TestH7MutationRollback:
    ...
    def test_mutation_source_has_rollback(self):
        ...
        assert "pre_mutation" in source, "mutate() missing pre_mutation rollback snapshot"
...
class TestH11CPCVValidation:
    ...
    def test_n_test_groups_zero_returns_empty(self):
        ...
        assert result == [], f"Expected empty list for n_test_groups=0, got {len(result)} paths"

    def test_n_groups_one_returns_empty(self):
        from genetic_algorithm.evaluation.cpcv import generate_cpcv_paths

        result = generate_cpcv_paths(n_groups=1, n_test_groups=1)
        assert result == [], f"Expected empty list for n_groups=1, got {len(result)} paths"
    ...
    def test_valid_inputs_return_paths(self):
        ...
        # C(6,2) = 15
        assert len(result) == 15, f"Expected 15 paths for C(6,2), got {len(result)}"
