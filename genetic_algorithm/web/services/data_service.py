...
Run identity and lifecycle come from the canonical SQLite catalog.  Files are
read only as hash-bound run artifacts or explicitly legacy UI data.
"""
...
class DataService:
    ...
    Active in-process handles come from RunManager. Historical run identity
    and lifecycle come only from the canonical catalog; files are consulted
    only after the catalog points at a specific artifact/config path.
    """

    def __init__(self, run_manager=None, *, state_path: str | Path | None = None) -> None:
    ...
    def list_runs(self) -> List[RunSummary]:
        """List in-process handles plus cataloged canonical/legacy experiments."""
        active = self.run_manager.list_runs()
        active_ids = {r.run_id for r in active}
        cataloged = [
            self._summary_from_catalog(record)
            for record in self.catalog.list_records()
            if record.experiment_id not in active_ids
        ]
        return active + cataloged
    ...
    def get_run_detail(self, run_id: str) -> Optional[RunDetail]:
        ...
        record = self.catalog.get_record(run_id)
        if record is None:
            return None
        summary = self._summary_from_catalog(record)
        config = self._load_catalog_config(record)
        return RunDetail(
            **summary.model_dump(),
            config=config,
            generation_stats=[],
            best_individual_id=record.best_candidate_id,
            mode=(
                "canonical_v2"
                if record.source == "CANONICAL_V2"
                else "legacy_import"
            ),
        )

    def _summary_from_catalog(self, record: ExperimentRecordV2) -> RunSummary:
        status_map = {
        ...
        return RunSummary(
            ...
            config_name=Path(record.config_path).stem if record.config_path else record.source,
            total_generations=int(ga.get("generations", 0) or 0),
            best_fitness=record.best_score,
            best_profit=(
                record.best_net_return * 100.0
                if record.best_net_return is not None
                else None
            ),
            population_size=int(ga.get("population_size", 0) or 0),
    ...
    @staticmethod
    def _load_catalog_config(record: ExperimentRecordV2) -> dict[str, Any]:
