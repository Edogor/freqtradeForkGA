...
def generate_report(benchmark_dir: Path) -> str:
    ...
    add("  LEGACY DIAGNOSTIC ONLY — NOT DECISION-ELIGIBLE")
    add("  Log fields are not candidate/scenario bound and must not rank runs.")
    ...
    add("-" * 72)
    perf_header = (
        f"{'Run':<30} {'Status':>8} {'Time':>8} {'BestFit':>8} "
    ...
    for run_name in active_runs:
        ...
        gens = (
            f"{m.get('gen_completed', '?')}/{m.get('gen_total', '?')}"
            if "gen_completed" in m
            else "N/A"
        )
    ...
    add("-" * 72)
    rob_header = f"{'Run':<30} {'HoldDeg':>8} {'Sharpe':>8} {'SAFE':>5} {'WARN':>5} {'OFIT':>5}"
    add(rob_header)
    add("-" * len(rob_header))
    ...
    add("AUTOMATED RANKING DISABLED")
    add("-" * 72)
    add("  These legacy fields may belong to different candidates or scenarios.")
    add("  Use wave_comparison.py with a reconciled V2 ANALYSIS decision.")
    add()
    add("=" * 72)
    add("  END OF REPORT")
    add("=" * 72)
...
def try_generate_charts(benchmark_dir: Path, all_data: Dict = None):
    """Refuse to chart log-derived fitness as a cross-run comparison."""
    del benchmark_dir, all_data
    print("  Comparison charts disabled: legacy log metrics are not candidate/scenario bound")
