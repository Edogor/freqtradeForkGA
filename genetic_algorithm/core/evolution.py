...
from genetic_algorithm.evaluation.panel_contract import build_evaluation_panel
...
class GeneticAlgorithm:
    ...
    def __init__(
        ...
        # Keep one validation contract even when tests or embedding code patch
        # the loader and provide an already-resolved mapping directly.
        from genetic_algorithm.config.schema import validate_config_or_raise

        validate_config_or_raise(self.config)
        ...
        # Canonical V2 workers use this explicit channel instead of the legacy
        # filesystem-discovery warm start, which intentionally remains
        # best-effort for backwards compatibility.
        self._strict_initial_seeds: List[Individual] = []
        self._immigrant_provider: Optional[
            Callable[["GeneticAlgorithm", int], List[Individual]]
        ] = None
        ...
        if self._lifecycle.enabled:
            ...
            self.logger.info(
                f"[LIFECYCLE] Strategy lifecycle manager enabled ({self._lifecycle.get_summary()})"
            )
    ...
    def _setup_evaluator(self):
        ...
        if self.regime_aware_enabled:
            self.fitness_evaluator = create_regime_aware_evaluator(self.config, auto_detect=True)
            self.logger.info("=" * 70)
        ...
        self.evaluation_panel = build_evaluation_panel(
            self.config,
            evaluator=self.fitness_evaluator,
        )
        self._evaluation_island: Optional[str] = self.config.get("_island_name")
        ...
        if self.parallel_enabled:
            if is_parallel_available():
                self.parallel_evaluator = ParallelEvaluator(
                    self.config, num_workers=parallel_config.get("num_workers")
                )
                self.logger.info("=" * 70)
                ...
                self.logger.warning(
                    "Parallel evaluation requested but multiprocessing not available"
                )
    ...
    def _setup_visualization(self, visualize: bool, interactive: bool):
        ...
        if visualize:
            try:
                ...
                plots_dir = Path(
                    output_config.get("plots_dir")
                    or Path(
                        output_config.get(
                            "dir",
                            output_config.get("directory", "genetic_algorithm/output"),
                        )
                    )
                    / "plots"
                )
        ...
        if trade_vis_config.get("enabled", False):
            try:
                ...
                self.logger.info(
                    f"TradeVisualizer initialized (mode={self.trade_vis_mode}, top_n={self.trade_vis_top_n})"
                )
            except ImportError as e:
    ...
    def _setup_hall_of_fame(self):
        ...
        self.hall_of_fame = HallOfFame(
            ...
            required_panel_id=self.evaluation_panel.panel_id,
            required_panel_role=self.evaluation_panel.role,
            panel_data_identity_verified=(self.evaluation_panel.data_identity_verified),
        )
    ...
    def _setup_llm(self):
        ...
        if self.progress_enabled:
            ...
            self.logger.warning(
                "Progress bar requested but tqdm not installed. Run: pip install tqdm"
            )
    ...
    def _get_common_indicators(self, population, top_n: int = 3):
        ...
        evaluated = [ind for ind in population.individuals if ind.has_measured_fitness]
        if not evaluated:
            return []
    ...
    def _setup_diagnostics(self):
        ...
        output_config = self.config.get("output", {})
        output_dir = Path(
            output_config.get("dir", output_config.get("directory", "genetic_algorithm/output"))
        )
    ...
    def _restore_from_legacy_checkpoint(self, checkpoint: Dict[str, Any]) -> Population:
        ...
        self.logger.info(
            f"[CHECKPOINT] Restored: generation={self.current_generation}, "
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        if not logger.handlers:
            ...
            if (
                not monitor_active
                and log_config.get("console", True)
                and (not log_file or stdout_is_tty)
            ):
    ...
    def save_checkpoint(
        self, population: "Population", generation: int, filepath: Optional[str] = None
    ) -> str:
        """
        ...
        extras = {
            ...
            if hasattr(self, "feature_tracker")
            else None,
            "holdout_state": {
        ...
        return self._checkpoint_mgr.save(
            ...
            provenance=self.config.get("checkpoint_provenance"),
        )
    ...
    def load_checkpoint(self, filepath: str) -> tuple:
        ...
        if ft_data and hasattr(self, "feature_tracker"):
            try:
                ...
                self.logger.info(
                    f"[CHECKPOINT] Restored feature tracker ({ft_data.get('total_generations', 0)} generations of data)"
                )
            except Exception as e:
        ...
        if me_data and self._map_elites.enabled:
            try:
                ...
                self.logger.info(
                    f"[CHECKPOINT] Restored MAP-Elites archive ({self._map_elites.filled_cells} cells)"
                )
            except Exception as e:
    ...
    def set_immigrant_provider(
        self, provider: Callable[["GeneticAlgorithm", int], List[Individual]]
    ):
        """
    ...
    def inject_immigrants(self, immigrants: List[Individual]):
        ...
        self.logger.info(
            f"[IMMIGRANTS] Queued {len(immigrants)} external immigrants for next generation"
        )

    def set_strict_initial_seeds(self, seeds: List[Individual]) -> None:
        """Bind validated initial genomes for a fail-closed V2 warm start.

        The supplied individuals are copied through the serialization contract,
        stripped of stale evaluation state, and required to fit in the initial
        population.  Unlike ``warm_start``, errors are never swallowed.
        """

        if len(seeds) > self.population_size:
            raise ValueError("strict initial seed count exceeds genetic_algorithm.population_size")
        copied: List[Individual] = []
        for index, source in enumerate(seeds):
            clone = Individual.from_dict(source.to_dict())
            clone.strategy_gene.generation = 0
            clone.strategy_gene.individual_id = 1_000_000 + index
            clone.strategy_gene.assign_instance_ids()
            clone.fitness = None
            clone.raw_fitness = None
            clone.objectives = None
            clone.rank = 0
            clone.crowding_distance = 0.0
            clone.metrics = {"origin": "v2_strict_initial_seed"}
            clone.evaluated = False
            clone.fitness_evidence = "UNMEASURED"
            clone.fitness_panel_id = None
            clone.fitness_panel_role = None
            clone.metrics["fitness_evidence"] = clone.fitness_evidence
            copied.append(clone)
        self._strict_initial_seeds = copied
    ...
    def request_shutdown(self):
        ...
        self.logger.info(
            "[SHUTDOWN] Graceful shutdown requested. Will save checkpoint after current generation."
        )
    ...
    def initialize_population(self) -> Population:
        ...
        # V2 seed binding is deliberately separate from the legacy warm-start
        # loader.  Any invalid source has already raised in
        # set_strict_initial_seeds(), and every accepted genome must appear in
        # the initial population.
        for seed in self._strict_initial_seeds:
            _enforce_min_entry_conditions(seed.strategy_gene, self.config)
            seed.strategy_gene.assign_instance_ids()
            population.add_individual(seed)
        if len(population) != len(self._strict_initial_seeds):
            raise RuntimeError("strict initial seeds were not injected completely")
        if self._strict_initial_seeds:
            self.logger.info(
                f"[V2-SEED] Injected {len(self._strict_initial_seeds)} strict initial genomes"
            )

        # ── Warm-start: inject individuals from a previous experiment ──
        warm_injected = 0
        from genetic_algorithm.core.warm_start import WarmStartError, WarmStartLoader

        ws_loader = WarmStartLoader(self.config, self.logger)
        ws_individuals = ws_loader.load()
        available_slots = self.population_size - len(population)
        if len(ws_individuals) > available_slots:
            raise WarmStartError(
                f"warm-start produced {len(ws_individuals)} genomes but only "
                f"{available_slots} population slots are available"
            )
        ...
        if self.llm_enabled and self.strategy_designer.enabled and remaining > 0:
            ...
            if llm_count > 0:
                ...
                for gene in llm_genes:
                    ...
                    if self.strategy_designer and hasattr(
                        self.strategy_designer, "_last_provider_used"
                    ):
                        individual.metrics["llm_provider"] = (
                            self.strategy_designer._last_provider_used
                        )
                    population.add_individual(individual)
        ...
        for i in range(random_remaining):
            strategy_gene = self.strategy_generator.generate_random_strategy(
                generation=0, individual_id=seed_start_id + llm_count + i
            )
            individual = Individual(strategy_gene=strategy_gene)
            population.add_individual(individual)

        self.logger.info(
            f"Population initialized: {hof_injected} hall-of-fame + {seed_count} seeded + {llm_count} LLM + {random_remaining} random"
        )
    ...
    def evaluate_population(self, population: Population):
        ...
        if self._surrogate.enabled and self._surrogate.ready:
            ...
            if skipped:
                self.logger.info(
                    f"[SURROGATE] Skipped {len(skipped)} candidates (surrogate-scored)"
                )
        ...
        if hasattr(evaluator, "run_deferred_validation"):
            ...
            if validated > 0:
                self.logger.info(
                    f"[EVAL] Deferred validation completed for {validated} top candidates"
                )
                self._refresh_nsga2_objectives(
                    population,
                    reason="deferred pair validation",
                )

        # Sequential, parallel and deferred evaluation all converge on this
        # boundary.  Stamp the exact panel only after every real backtest path
        # has finished; surrogate-only candidates are deliberately excluded.
        self._stamp_evaluation_panel(unevaluated)

    def _stamp_evaluation_panel(self, individuals: List[Individual]) -> int:
        """Bind all real evaluator outcomes to this GA's exact panel."""
        stamped = 0
        for individual in individuals:
            if individual.fitness_evidence in {
                "BACKTEST_VALID",
                "BACKTEST_FAILED",
            }:
                individual.assign_fitness_panel(
                    self.evaluation_panel.panel_id,
                    self.evaluation_panel.role,
                )
                if self._evaluation_island:
                    individual.metrics["evaluation_island"] = self._evaluation_island
                stamped += 1
        return stamped

    def _refresh_nsga2_objectives(
        self,
        population: "Population",
        *,
        reason: str,
    ) -> int:
        """Rebuild objective vectors after a metric-changing validation pass.

        Sequential and parallel base evaluation both create objectives at the
        individual boundary.  Deferred validation may subsequently replace an
        individual's metrics, so retaining the old vector would select on
        training metrics while reporting validation metrics.
        """
        if self.mode != "nsga2":
            return 0

        refreshed = 0
        min_trades = self.nsga2_config.get("min_trades", 0)
        for individual in population:
            if not individual.evaluated or not individual.metrics:
                continue
            ...
            try:
                ...
                raise RuntimeError(
                    f"NSGA-II objective refresh failed after {reason} for {individual.id}: {exc}"
                ) from exc
            individual.set_objectives(objectives, individual.metrics)
            refreshed += 1

        self.logger.info(
            "[NSGA-II] Refreshed %d objective vectors after %s",
            refreshed,
            reason,
        )
        return refreshed
    ...
    def get_state_snapshot(self) -> dict:
        ...
        return {
            ...
            "best_profit": self.best_individual.metrics.get("profit")
    ...
    def _post_hoc_walk_forward_validation(self, population: "Population"):
        """Assert that parallel walk-forward already used canonical semantics."""
        wf_config = self.config.get("walk_forward", {})
        if not wf_config.get("enabled", False):
            return 0
        if not self.parallel_enabled:
            return 0  # WF already ran inside sequential evaluation
        if getattr(self.parallel_evaluator, "walk_forward_in_workers", False):
            # Current parallel workers run the exact FitnessEvaluator WF path.
            # Re-running the old flat/top-K approximation would double-spend
            # budget and execute different failure, gap and aggregation rules.
            return 0

        raise RuntimeError(
            "parallel walk-forward evaluator does not guarantee in-worker "
            "sequential-parity semantics"
        )
    ...
    def _run_holdout_test(self, population: "Population", holdout_config: Dict):
        ...
        candidates = population.get_best_measured(top_n)
        if not candidates:
            return
    ...
    def _run_post_evolution_cpcv(self, population, cpcv_config: dict):
        ...
        self.logger.info(f"{'─' * 70}")
        self.logger.info(
            f"CPCV POST-EVOLUTION VALIDATION ({len(candidates)} strategies, {n_groups} blocks)"
        )
        self.logger.info(f"{'─' * 70}")
        ...
        try:
            ...
            if total_days < n_groups * 7:
                self.logger.warning(
                    f"[CPCV] Timerange too short ({total_days} days) for {n_groups} blocks"
                )
                return
        except Exception as e:
        ...
        for i in range(n_groups):
            ...
            b_end = (
                start_dt + timedelta(days=int((i + 1) * block_days)) if i < n_groups - 1 else end_dt
            )
        ...
        try:
            ...
            for sid, stats in per_strategy.items():
                self.logger.info(
                    f"  {sid}: OOS mean={stats['mean_oos']:.2f}%, "
    ...
    def _update_diversity_references(self, population: "Population"):
        ...
        try:
            ...
            top = population.get_best_measured(max(self.elite_size, 5))
            vectors = []
            for ind in top:
    ...
    def _run_holdout_monitoring(self, population: "Population", generation: int):
        ...
        candidates = population.get_best_measured(self.holdout_monitoring_top_n)
        if not candidates:
            return

        self.logger.info(
            f"[HOLDOUT-MON] Gen {generation + 1}: evaluating top-{len(candidates)} on {holdout_range}..."
        )
    ...
    def _apply_global_dsr_penalties(self, population):
        ...
        evaluated = [
            ind for ind in population.individuals if ind.has_measured_fitness and ind.metrics
        ]
        if not evaluated:
            return
    ...
    def _evaluate_population_parallel(self, unevaluated: list):
        ...
        result = self.parallel_evaluator.evaluate_batch(
            unevaluated, progress_callback=progress_callback if self.progress_enabled else None
        )
    ...
    def _evaluate_population_sequential(self, unevaluated: list):
        ...
        for i, individual in iterator:
            ...
            if not self.progress_enabled:
                if i == 0 or (i + 1) % 5 == 0 or i == len(unevaluated) - 1:
                    self.logger.info(
                        f"[EVAL] Progress: {i + 1}/{len(unevaluated)} ({((i + 1) / len(unevaluated) * 100):.0f}%)"
                    )

            try:
                # Evaluate fitness
                fitness, metrics = self.fitness_evaluator.evaluate(
                    individual.strategy_gene, strategy_name=strategy_name
                )
                ...
                self.logger.debug(
                    f"  {strategy_name}: fitness={fitness:.4f}, profit={metrics.get('profit', 0):.2f}%"
                )
        ...
        self.logger.info(
            f"[EVAL] Complete: {successful} succeeded, {failed} failed, avg profit: {avg_profit:.2f}%"
        )
    ...
    def _should_update_best_individual(self, candidate: Individual) -> bool:
        ...
        if not candidate.has_measured_fitness or candidate.raw_fitness is None:
            return False
    ...
    def _visualize_strategy_trades(
        self, individual: Individual, generation: int, individual_idx: int
    ):
        """
        ...
        try:
            ...
            if not result.success:
                self.logger.warning(
                    f"[TRADE VIS] Backtest failed for {strategy_name}: {result.error_message}"
                )
                return
            ...
            if saved_files:
                self.logger.info(
                    f"[TRADE VIS] Generated {len(saved_files)} chart(s) for {strategy_name}"
                )
    ...
    def create_next_generation(self, population: Population) -> Population:
        ...
        if self.mode == "nsga2":
            # True NSGA-II lifecycle: generate λ offspring, evaluate every new
            # candidate through the same canonical path, then perform (μ+λ)
            # environmental selection.  Selecting inside GenerationStep used
            # to happen while every child still had objectives=None, causing
            # the μ parents to occupy all survivor slots forever.
            self.logger.info(
                "[NSGA-II] Evaluating %d offspring before (μ+λ) selection",
                len(next_gen.individuals),
            )
            self.evaluate_population(next_gen)
            next_gen = self._generation_step.select_nsga2_survivors(
                population,
                next_gen,
            )
        return next_gen
    ...
    def process_generation(self, population: "Population", gen: int, pareto_archive=None):
        ...
        if self._aos.enabled:
            for ind in population:
                if not ind.has_measured_fitness:
                    continue
                m = ind.metrics or {}
        ...
        if self._map_elites.enabled:
            ...
            if bonus_count:
                self.logger.info(
                    f"[MAP-ELITES] Applied exploration bonus to {bonus_count} individuals"
                )
        ...
        if self.mode == "nsga2":
            ...
            self.logger.info(
                f"[NSGA-II] {len(fronts)} Pareto fronts, front 1 has {len(pareto_front)} individuals"
            )
            ...
            if self.fitness_sharing:
                apply_fitness_sharing(
                    population, sigma_share=self.sharing_radius, distance_matrix=distance_matrix
                )
        ...
        measured_best = population.get_best_measured(1)
        best = measured_best[0] if measured_best else None
        if best is not None and self._should_update_best_individual(best):
        ...
        if self.trade_visualizer and self.trade_vis_mode == "each_generation":
            top_individuals = population.get_best_measured(self.trade_vis_top_n)
        ...
        try:
            ...
            if indicator_weights:
                ...
                self.logger.debug(
                    f"[FEATURE-IMPORTANCE] Updated indicator weights: "
        ...
        try:
            ...
            if (
                self.holdout_early_stop
                and self._holdout_consecutive_bad >= self.holdout_early_stop_checks
            ):
                self.logger.info(
        ...
        try:
            ...
            _extras["avg_holdout_penalty"] = (
                round(sum(penalised) / len(penalised), 4) if penalised else 0.0
            )
            unused_counts = [
                ind.metrics.get("unused_indicators", 0) for ind in all_inds if ind.metrics
            ]
            _extras["avg_unused_indicators"] = round(
                sum(unused_counts) / max(len(unused_counts), 1), 2
            )
    ...
    def advance_generation(self, population: "Population", gen: int, stats) -> tuple:
        ...
        if self.check_convergence(stats):
            ...
            self.monitor.on_log(
                f"Evolution converged early at gen {gen + 1}/{self.generations}",
                "warning",
            )
