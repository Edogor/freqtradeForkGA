"""Lease-aware scheduler for immutable V2 shadow replay worker specs."""
...
class ShadowSchedulerError(RuntimeError):
    """Raised when scheduler state cannot safely make progress."""


class ShadowSchedulerTimeout(ShadowSchedulerError):
    """Raised when run_until_idle reaches its explicit time budget."""


class ShadowSchedulerConfigV2(StrictV2Model):
    ...
    def _coherent_runtime(self) -> ShadowSchedulerConfigV2:
        if self.heartbeat_interval_seconds >= self.lease_seconds:
            raise ValueError("heartbeat interval must be shorter than lease")
        if (
            self.max_execution_seconds is not None
            and self.max_execution_seconds <= 0
        ):
            raise ValueError("max_execution_seconds must be positive or null")
        executable = Path(self.python_executable).absolute()
        if not executable.is_file():
            raise ValueError("python_executable does not exist")
        object.__setattr__(self, "python_executable", str(executable))
        return self


class SchedulerFutureErrorV2(StrictV2Model):
    attempt_id: str = Field(min_length=1)
    error_type: str = Field(min_length=1)
    detail: str = Field(min_length=1)


class ShadowSchedulerTickV2(StrictV2Model):
    ...
    future_errors: list[SchedulerFutureErrorV2] = Field(default_factory=list)
    reconciliation: list[ReconciliationRecordV2] = Field(default_factory=list)
...
def queue_prepared_replay_attempt(
    ...
    *,
    ...
    priority: int = 0,
    retry_number: int = 0,
    actor: str = "shadow-controller-v2",
    python_executable: str | Path = sys.executable,
    working_directory: str | Path,
) -> AttemptStateV2:
    """Register, bind, validate, and queue one replay attempt idempotently."""

    if not bound_at <= validated_at <= queued_at:
        raise ShadowSchedulerError("bind/validate/queue timestamps are not monotonic")
    ...
    if loaded.manifest != manifest or prepared.spec.attempt_id != manifest.attempt_id:
        raise ShadowSchedulerError("prepared replay worker differs from attempt manifest")
    binding = WorkerBindingV2(
        worker_kind=WorkerKind.SHADOW_REPLAY,
        argv=replay_worker_argv(
            prepared,
    ...
    if state.status == AttemptLifecycleStatus.VALIDATED:
        state = state_store.transition(
            ...
            actor=actor,
            reason="QUEUED_FOR_SHADOW_REPLAY",
        )
...
class ShadowSchedulerV2:
    """Run prepared replay attempts without consulting the legacy JSON registry."""
    ...
    def __init__(
        ...
        *,
        ...
        if not scheduler_id:
            raise ShadowSchedulerError("scheduler_id cannot be empty")
    ...
    def __enter__(self) -> ShadowSchedulerV2:
        return self
    ...
    def queue_replay(
        ...
        *,
        ...
        if self._closed:
            raise ShadowSchedulerError("scheduler is closed")
        return queue_prepared_replay_attempt(
            self.state_store,
            manifest=manifest,
            prepared=prepared,
            bound_at=bound_at,
    ...
    def run_until_idle(self, *, timeout_seconds: float = 60.0) -> list[ShadowSchedulerTickV2]:
        if timeout_seconds <= 0:
            raise ShadowSchedulerError("timeout_seconds must be positive")
    ...
    def _execute_claimed(self, claimed: AttemptStateV2) -> AttemptStateV2:
        ...
        if binding is None or binding.worker_kind != WorkerKind.SHADOW_REPLAY:
            raise AttemptStateError("claimed shadow attempt lacks replay worker binding")
