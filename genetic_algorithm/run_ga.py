...
def check_disk_space(min_gb: float = 5.0, path: str = ".") -> bool:
    ...
    try:
        ...
        if free_gb < min_gb:
            ...
            print("   Free up space by cleaning old caches/checkpoints, then retry.")
            return False
...
def resolve_output_dir(config: dict) -> Path:
    """Resolve legacy report output from config only.

    Canonical workers persist their absolute attempt-local value in
    ``output.dir``.  Environment overrides were removed because they were not
    hash-bound and could silently redirect a queued run after validation.
    """

    return Path(config.get("output", {}).get("dir") or OUTPUT_DIR)
...
def setup_logging(monitor_active: bool = False):
    ...
    file_handler = RotatingFileHandler(
        log_file,
        maxBytes=20_000_000,
        backupCount=5,  # 20MB per file, keep 5 backups
    )
...
def print_configuration(config: dict):
    ...
    print(
        ...
        else "  Mutation Rate:      (default)"
    )
    crossover_rate = ga_config.get("crossover_rate")
    print(
        f"  Crossover Rate:     {crossover_rate:.2%}"
        if crossover_rate is not None
        else "  Crossover Rate:     (default)"
    )
    ...
    print(
        f"  Stake Amount:       {stake}" if stake is not None else "  Stake Amount:       (default)"
    )
    fee = backtest_config.get("fee")
    print(
        f"  Fee:                {fee:.3%}" if fee is not None else "  Fee:                (default)"
    )
    max_trades = backtest_config.get("max_open_trades")
    print(
        f"  Max Open Trades:    {max_trades}"
        if max_trades is not None
        else "  Max Open Trades:    (default)"
    )
    print()
    ...
    for fw_key, fw_label in [
        ...
        ("monthly_stability", "Monthly Stability"),
        ("cross_pair", "Cross Pair"),
    ]:
...
def _measured_finalists(individuals: list) -> list:
    """Remove search-only estimates from every legacy final-output boundary."""
    return [ind for ind in individuals if ind.has_measured_fitness]


def _require_measured_finalists(individuals: list, operation: str) -> None:
    invalid = [ind.id for ind in individuals if not ind.has_measured_fitness]
    if invalid:
        raise ValueError(
            f"{operation} refuses finalists without successful real-backtest evidence: {invalid}"
        )
...
def print_top_strategies(top_strategies: list, strategy_generator: StrategyGenerator):
    ...
    """
    _require_measured_finalists(top_strategies, "top-strategy reporting")
...
def save_top_strategies(
    top_strategies: list, strategy_generator: StrategyGenerator, output_dir: Path
):
    """
    ...
    if not SAVE_STRATEGIES:
        return
    _require_measured_finalists(top_strategies, "strategy export")
    ...
    for rank, individual in enumerate(top_strategies, 1):
        ...
        # Create filename
        filename = (
            f"strategy_rank{rank}_gen{gene.generation}_ind{gene.individual_id}_{timestamp}.py"
        )
        ...
        print(
            f"      Fitness: {individual.fitness:.4f}, Profit: {_safe_metric(individual.metrics, 'profit'):.2f}%"
        )
...
def save_summary_report(
    ...
    generation_holdout_history=None,
):
    """
    ...
    """
    _require_measured_finalists(top_strategies, "summary export")
    ...
    with open(report_path, "w") as f:
        ...
        for rank, individual in enumerate(top_strategies, 1):
            ...
            f.write(
                f"  Raw Fitness: {(individual.raw_fitness if individual.raw_fitness is not None else 0):.4f}\n"
            )
            ...
            if holdout_fitness is not None:
                ...
                f.write(
                    f"  Holdout Degradation: {_safe_metric(metrics, 'holdout_degradation'):.1%}\n"
                )
            ...
            if assessment.overall_label != "UNKNOWN":
                f.write(
                    f"  Overfit Risk: {assessment.overall_label} "
        ...
        if assessments:
            ...
            f.write(
                f"  SAFE: {labels.count('SAFE')}  WARNING: {labels.count('WARNING')}  "
...
def _save_evolution_stats(
    output_dir: Path, config: dict, generation_stats, generation_holdout_history=None
):
...
Examples:
  ...
  python genetic_algorithm/run_ga.py --visualize --no-interactive
    ...
    parser.add_argument(
        "--port", type=int, default=8501, help="Port for the web dashboard (default: 8501)"
    )
...
def _legacy_main_disabled():
    """Retained temporarily for report helpers; operational starts must not call it."""
    # Parse arguments
    ...
    try:
        ...
        logger.debug(f"Preflight check unavailable: {exc}")

    if args.validate_only:
        print("✅ Config validation passed!")
        return 0

    # This function is retained only as non-dispatched reference code.  It must
    # never register lifecycle state: canonical execution is owned by main()
    # through runner_v2 and the SQLite attempt state machine.
    experiment_id = None
    registry = None
    ...
    # Derive output from the resolved config.  V2 workers bind this path to the
    # immutable attempt spec; an environment variable must not override it.
    ...
    try:
        ...
        if gim_cfg.get("enabled", False):
            ...
            n_gens = gim_cfg.get(
                "generations", config.get("genetic_algorithm", {}).get("generations", 20)
            )
            ...
            result_batch = extract_island_finalists(
                results,
                expected_islands=[
                    island_config.name
                    for island_config in generic_island_evo.island_configs
                ],
            )
            top_strategies = list(result_batch.top(TOP_STRATEGIES_COUNT))
        ...
        # ── Island Model branch (regime-locked) ──
            ...
            result_batch = extract_island_finalists(
                results,
                expected_islands=[
                    island_config.name for island_config in island_evo.island_configs
                ],
            )
            top_strategies = list(result_batch.top(TOP_STRATEGIES_COUNT))
            ...
            ga = GeneticAlgorithm(
                tmp_config_path, visualize=args.visualize, interactive=not args.no_interactive
            )
    ...
    measured_top = _measured_finalists(top_strategies)
    rejected_finalists = len(top_strategies) - len(measured_top)
    if rejected_finalists:
        logger.warning(
            "[FINAL] Rejected %d candidates without successful real-backtest evidence",
            rejected_finalists,
        )
    top_strategies = measured_top
    if not top_strategies:
        logger.error("[FINAL] Evolution produced no measured finalist")
        if registry and experiment_id:
            try:
                registry.fail(experiment_id, error="no measured finalist")
            except Exception:
                pass
        return 1
    ...
    try:
        ...
        if holdout_enabled and top_strategies:
            ...
            if original_timerange:
                ...
                for rank, individual in enumerate(top_strategies, 1):
                    ...
                    holdout_fitness_str = (
                        f"{holdout_fitness:.4f}" if holdout_fitness is not None else "N/A"
                    )
                    ...
                    print(
                        f"      Holdout: profit={holdout_metrics.get('profit', 0):.2f}%, "
                ...
                print()
                print(
                    "  Legend: ✓ = <30% degradation (robust), ⚠️  = ≥30% degradation (potential overfit)"
                )
                print()
            else:
        ...
        if cpcv_enabled and top_strategies:
            if ga is None:
                ...
                for rank, individual in enumerate(top_strategies, 1):
                    ...
                    if timerange and "-" in timerange:
                        ...
                        for block_idx in range(n_groups):
                            ...
                            block_end = (
                                start_date + timedelta(days=(block_idx + 1) * block_days)
                                if block_idx < n_groups - 1
                                else end_date
                            )
                            block_tr = (
                                f"{block_start.strftime('%Y%m%d')}-{block_end.strftime('%Y%m%d')}"
                            )
                        ...
                        logger.info(
                            f"  Rank {rank}: block profits = {[f'{p:.2f}' for p in block_profits]}"
                        )
                    else:
                ...
                if len(strategy_block_results) >= 2:
                    ...
                    if not skipped:
                        ...
                        for sid, oos_info in per_strategy_oos.items():
                            print(
                                f"      {sid}: mean_oos={oos_info['mean_oos']:.3f}, "
                        ...
                        for rank, individual in enumerate(top_strategies, 1):
                            ...
                            if oos_key in per_strategy_oos:
                                individual.metrics["cpcv_mean_oos"] = per_strategy_oos[oos_key][
                                    "mean_oos"
                                ]
                    else:
                ...
                print()
                print(
                    "  Legend: ✓ = PBO below threshold (low overfit risk), ⚠️ = PBO above threshold"
                )
                print()
            ...
            if ga is None:
                ...
                for rank, individual in enumerate(top_strategies, 1):
                    ...
                    bt_result = mc_backtester.backtest_strategy_with_trades(
                        strategy_code, strategy_name, strategy_max_open_trades=gene.max_open_trades
                    )
                    ...
                    if bt_result.success and bt_result.trades:
                        ...
                        print(
                            f"  {status} Rank {rank}: robustness={mc_result.robustness_score:.1%}, "
        ...
        if registry and experiment_id:
            try:
                best_finalist = top_strategies[0]
                _best_fitness = float(best_finalist.raw_fitness)
                _best_profit = float(best_finalist.metrics["profit"])
                registry.complete(
                    experiment_id, best_fitness=_best_fitness, best_profit=_best_profit
                )
...
def _start_with_dashboard(args):
    ...
    config = load_and_update_config(config_file)
    config["experiment_name"] = config_file.stem
...
def main(argv=None):
    """Compatibility entry point delegating every evolution start to GA V2."""

    args = parse_arguments(argv)
    if args.validate_only:
        from genetic_algorithm.cli import main as cli_main

        return cli_main(["config", "validate", args.config, "--strict"])

    unsupported = [
        flag
        for enabled, flag in (
            (args.resume is not None, "--resume"),
            (args.seed is not None, "--seed"),
            (args.visualize, "--visualize"),
            (args.no_interactive, "--no-interactive"),
            (args.monitor_mode is not None, "--monitor-mode"),
            (args.dashboard, "--dashboard"),
            (args.dashboard_only, "--dashboard-only"),
        )
        if enabled
    ]
    if unsupported:
        print(
            ...
            + ", ".join(unsupported)
            + "; the disabled legacy runner was not started."
        )
        return 2

    from genetic_algorithm.cli import main as cli_main

    argv = ["run", args.config, "--yes"]
    if args.no_monitor:
